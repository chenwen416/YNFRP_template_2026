...
由于字体文件体积较大，且部分字体可能受版权或授权协议限制，本仓库不建议直接上传或公开分发字体文件。

⚠️注意：‘仿宋GB2312.ttf’文件在根目录里，记得在你的本地把‘仿宋GB2312.ttf’放在项目根目录下的 `fonts/` 文件夹内！！！

本模板默认从项目根目录下的 `fonts/` 文件夹调用字体。若你获得的是 `fonts.zip`，请在使用前将其解压缩，并将解压后的 `fonts/` 文件夹放到项目根目录下，例如：
...
请使用者自行确认所用字体文件的合法授权范围。本模板作者不提供、不分发、不授权任何商业字体文件。
...
- 下载：https://github.com/chenwen416/YNFRP_template_2026
...
需要局部强调时，可使用 \textbfk{关键技术指标 S/N > 10}，其中汉字为方正楷体 GBK，英文、数字和符号保持正常加粗。
```

如果希望某一个 `\subsection{...}` 标题使用类似 `\textbfk{...}` 的楷体加粗效果，可在可选参数中写 `kai` 或 `bfk`：

```latex
\subsection[kai]{面向云南本地 VLBI 台站的技术积累与推广应用}
```

若希望后续所有 `\subsection{...}` 都使用楷体加粗，可打开全局开关：

```latex
\YNSubsectionKaiBoldtrue
```

若要恢复默认仿宋加粗，使用：

```latex
\YNSubsectionKaiBoldfalse
```
